...
btnSave.addEventListener('click', ()=> {
    ...
    fs.appendFileSync(pathNameContact , name+" "+phone+'\n', "UTF-8",{'flags': 'a+'})
...
btnCall.addEventListener('click', ()=> {
    var today = new Date();
    var date = today.getFullYear()+'-'+(today.getMonth()+1)+'-'+today.getDate()+'__'+today.getHours()+':'+today.getMinutes()+':'+today.getSeconds();
    let phone = output.value;
    fs.appendFileSync(pathNameHistory , phone+" "+date+'\n', "UTF-8",{'flags': 'a+'})
     output.removeAttribute("value");
 });